...
interface PartyCardNewafonProps {
  city?: PartyCity;
  events?: PartyEvent[];
  sliderItems?: PartySliderItem[];
}

const PartyCardNewafon: React.FC<PartyCardNewafonProps> = ({ city, events, sliderItems }) => {
    ...
    <section className={cardStyles.partyCard}>
      ...
      <div className={cardStyles.contentSection}>
        ...
        <div className={cardStyles.cityTitle}>
          <h2 className={cardStyles.cityTitleText}>{city?.name || t('cities.newAfon')}</h2>
        </div>

        {/* Events Container */}
        <div className={cardStyles.eventsContainer}>
          {events && events.filter(event => event.city_slug === 'newafon').length > 0 ? (
            events
              .filter(event => event.city_slug === 'newafon')
              .slice()
          ...
          {/* Slider Block */}
          {sliderItems && sliderItems.filter(item => item.city_slug === 'newafon').length > 0 && (
            <div className={cardStyles.adBlock}>
              <AdSlider 
                items={sliderItems
                  .filter(item => item.city_slug === 'newafon')
                  .map(item => ({
