...
interface PartyCardGulripshProps {
  city?: PartyCity;
  events?: PartyEvent[];
    ...
    <section className={cardStyles.partyCard}>
      ...
      <div className={cardStyles.contentSection}>
        ...
        <div className={cardStyles.eventsContainer}>
          {events && events.filter(event => event.city_slug === 'gulripsh').length > 0 ? (
            events
              .filter(event => event.city_slug === 'gulripsh')
              .slice()
          ...
          {/* Slider Block */}
          {sliderItems && sliderItems.filter(item => item.city_slug === 'gulripsh').length > 0 && (
            <div className={cardStyles.adBlock}>
              <AdSlider 
                items={sliderItems
                  .filter(item => item.city_slug === 'gulripsh')
                  .map(item => ({
...
export default PartyCardGulripsh; 