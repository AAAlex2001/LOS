...
interface PartyCardGudautaProps {
  city?: PartyCity;
  events?: PartyEvent[];
  sliderItems?: PartySliderItem[];
}

const PartyCardGudauta: React.FC<PartyCardGudautaProps> = ({ city, events, sliderItems }) => {
    ...
    <section className={cardStyles.partyCard}>
      ...
      <div className={cardStyles.contentSection}>
        ...
        <div className={cardStyles.cityTitle}>
          <h2 className={cardStyles.cityTitleText}>{city?.name || t('cities.gudauta')}</h2>
        </div>

        {/* Events Container */}
        <div className={cardStyles.eventsContainer}>
          {events && events.filter(event => event.city_slug === 'gudauta').length > 0 ? (
            events
              .filter(event => event.city_slug === 'gudauta')
              .slice()
          ...
          {/* Slider Block */}
          {sliderItems && sliderItems.filter(item => item.city_slug === 'gudauta').length > 0 && (
            <div className={cardStyles.adBlock}>
              <AdSlider 
                items={sliderItems
                  .filter(item => item.city_slug === 'gudauta')
                  .map(item => ({
...
export default PartyCardGudauta; 