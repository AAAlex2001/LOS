...
const BeautySalonsSukhum: React.FC = () => {
  ...
  React.useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/beauty-salons/page/city/${encodeURIComponent('Сухум')}/`, { cache: 'no-store' });
    ...
    <div className={styles.pageWrapper}>
      ...
      <main className={styles.mainContent}>
        <section className={styles.titleSection}>
          <h1 className={styles.mainTitle}>{data?.title || 'Сухум: салоны красоты'}</h1>
        </section>
