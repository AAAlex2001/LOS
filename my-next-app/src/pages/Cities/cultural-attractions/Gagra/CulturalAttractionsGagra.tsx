...
const CulturalAttractionsGagra: React.FC = () => {
  ...
  React.useEffect(() => {
    const load = async () => {
      try {
        const url = getApiUrl(`/api/cultural-attractions/page/city_page/${encodeURIComponent('Гагра')}/`, locale);
    ...
    <div className={styles.pageWrapper}>
      ...
      <main className={styles.mainContent}>
        <section className={styles.titleSection}>
          <h1 className={styles.mainTitle}>{data?.title || 'Гагра: культурные достопримечательности'}</h1>
        </section>
            ...
            <div key={attraction.id} className={styles.attractionItem}>
              ...
              {attraction.working_hours && (
                <div className={styles.attractionWorkingHours}>
                  <span className={styles.workingHoursLabel}>{t('common.workingHours')}:</span>
                  <span className={styles.workingHoursValue}>{attraction.working_hours}</span>
                </div>
              )}
              
              <div className={styles.attractionAddress}>
                <span className={styles.addressLabel}>{t('common.address')}:</span>
