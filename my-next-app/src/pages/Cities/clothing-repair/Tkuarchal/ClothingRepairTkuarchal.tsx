...
import styles from './ClothingRepairTkuarchal.module.scss';
...
const ClothingRepairTkuarchal: React.FC = () => {
  ...
  React.useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/clothing-repair/page/city_page/${encodeURIComponent('Ткуарчал')}/`, { cache: 'no-store' });
      ...
      <main className={styles.mainContent}>
        <div className={styles.buildingsWrapper}>
          ...
          <section className={styles.titleSection}>
            <h1 className={styles.mainTitle}>{data?.title || 'Ткуарчал: ремонт одежды и обуви'}</h1>
          </section>
...
export default ClothingRepairTkuarchal;
