...
const ClothingRepairGulripsh: React.FC = () => {
  ...
  React.useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/clothing-repair/page/city_page/${encodeURIComponent('Гулрыпш')}/`, { cache: 'no-store' });
      ...
      <main className={styles.mainContent}>
        <div className={styles.buildingsWrapper}>
          ...
          <section className={styles.titleSection}>
            <h1 className={styles.mainTitle}>{data?.title || 'Гулрыпш: ремонт одежды и обуви'}</h1>
          </section>
...
export default ClothingRepairGulripsh;
