...
import styles from './ClothingRepairGal.module.scss';
...
const ClothingRepairGal: React.FC = () => {
  ...
  React.useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/clothing-repair/page/city_page/${encodeURIComponent('Гал')}/`, { cache: 'no-store' });
      ...
      <main className={styles.mainContent}>
        <div className={styles.buildingsWrapper}>
          ...
          <section className={styles.titleSection}>
            <h1 className={styles.mainTitle}>{data?.title || 'Гал: ремонт одежды и обуви'}</h1>
          </section>
...
export default ClothingRepairGal;
