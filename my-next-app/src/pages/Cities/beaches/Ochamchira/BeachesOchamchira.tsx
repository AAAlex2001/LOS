...
const BeachesOchamchira: React.FC = () => {
  ...
  React.useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/beaches/page/city_page/${encodeURIComponent('Очамчыра')}/`, { cache: 'no-store' });
    ...
    <div className={styles.pageWrapper}>
      ...
      <main className={styles.mainContent}>
        <section className={styles.titleSection}>
          <h1 className={styles.mainTitle}>{data?.title || 'Очамчыра: пляжи'}</h1>
        </section>
