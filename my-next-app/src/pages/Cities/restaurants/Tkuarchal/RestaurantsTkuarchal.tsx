...
const RestaurantsTkuarchal: React.FC = () => {
  ...
  React.useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/restaurants/page/city_page/${encodeURIComponent('Ткуарчал')}/`, { cache: 'no-store' });
    ...
    <div className={styles.pageWrapper}>
      ...
      <main className={styles.mainContent}>
        <section className={styles.titleSection}>
          <h1 className={styles.mainTitle}>{data?.title || 'Ткуарчал: рестораны'}</h1>
        </section>
