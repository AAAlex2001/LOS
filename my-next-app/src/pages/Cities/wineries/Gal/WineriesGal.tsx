...
import styles from './WineriesGal.module.scss';
...
const WineriesGal: React.FC = () => {
  ...
  React.useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/wineries/page/city_page/${encodeURIComponent('Гал')}/`, { cache: 'no-store' });
      ...
      <main className={styles.mainContent}>
        <div className={styles.buildingsWrapper}>
          ...
          <section className={styles.titleSection}>
            <h1 className={styles.mainTitle}>{data?.title || 'Гал: винодельни'}</h1>
          </section>
