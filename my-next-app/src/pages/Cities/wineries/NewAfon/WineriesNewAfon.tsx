...
import styles from './WineriesNewAfon.module.scss';
...
const WineriesNewAfon: React.FC = () => {
  ...
  React.useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/wineries/page/city_page/${encodeURIComponent('Новый Афон')}/`, { cache: 'no-store' });
      ...
      <main className={styles.mainContent}>
        <div className={styles.buildingsWrapper}>
          ...
          <section className={styles.titleSection}>
            <h1 className={styles.mainTitle}>{data?.title || 'Новый Афон: винодельни'}</h1>
          </section>
...
export default WineriesNewAfon;
