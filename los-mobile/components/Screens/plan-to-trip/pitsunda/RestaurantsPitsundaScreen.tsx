...
export default function RestaurantsPitsundaScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/restaurants/page/city_page/${encodeURIComponent(t('cities.pitsunda'))}/`), { cache: 'no-store' });
