...
export default function HotelsPitsundaScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/hotels/page/city_page/${encodeURIComponent('Пицунда')}/`, { cache: 'no-store' });
