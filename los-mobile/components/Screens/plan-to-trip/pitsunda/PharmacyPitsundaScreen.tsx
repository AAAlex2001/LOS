...
export default function PharmacyPitsundaScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/pharmacy/page/city_page/${encodeURIComponent('Пицунда')}/`, { cache: 'no-store' });
