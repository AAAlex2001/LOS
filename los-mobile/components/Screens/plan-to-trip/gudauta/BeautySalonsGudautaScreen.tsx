...
export default function BeautySalonsGudautaScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/beauty-salons/page/city_page/${encodeURIComponent(t('cities.gudauta'))}/`), { cache: 'no-store' });
