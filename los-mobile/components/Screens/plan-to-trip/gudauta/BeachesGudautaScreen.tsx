...
export default function BeachesGudautaScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/beaches/page/city_page/${encodeURIComponent(t('cities.gudauta'))}/`), { cache: 'no-store' });
