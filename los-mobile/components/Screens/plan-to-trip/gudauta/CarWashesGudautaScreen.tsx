...
export default function CarWashesGudautaScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/car-washes/page/city_page/${encodeURIComponent('Гудаута')}/`, { cache: 'no-store' });
