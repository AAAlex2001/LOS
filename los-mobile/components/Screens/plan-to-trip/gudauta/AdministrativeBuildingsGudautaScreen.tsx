...
export default function AdministrativeBuildingsGudautaScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/administrative-buildings/page/city_page/${encodeURIComponent('Гудаута')}/`, { cache: 'no-store' });
