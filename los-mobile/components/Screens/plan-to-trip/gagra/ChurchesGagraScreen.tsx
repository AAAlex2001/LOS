...
export default function ChurchesGagraScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/churches/page/city_page/${encodeURIComponent(t('cities.gagra'))}/`), { cache: 'no-store' });
              ...
              <View key={church.id} style={styles.card}>
                {church.image_url && (
                  <View style={styles.imageContainer}>
                    <Image 
                      source={{ uri: toImageUrl(church.image_url) }}
