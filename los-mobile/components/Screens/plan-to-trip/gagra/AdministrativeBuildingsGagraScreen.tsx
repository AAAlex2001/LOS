...
export default function AdministrativeBuildingsGagraScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/administrative-buildings/page/city_page/${encodeURIComponent(t('cities.gagra'))}/`), { cache: 'no-store' });
