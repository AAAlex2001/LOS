...
export default function CulturalAttractionsGagraScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/cultural-attractions/page/city_page/${encodeURIComponent(t('cities.gagra'))}/`), { cache: 'no-store' });
