...
export default function CarWashesNewAfonScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/car-washes/page/city_page/${encodeURIComponent('Новый Афон')}/`, { cache: 'no-store' });
                ...
                <View style={styles.infoContainer}>
                  ...
                  {carWash.contacts && (
                    <View style={styles.infoBlock}>
                      ...
                      {extractPhoneNumber(carWash.contacts) ? (
                        <TouchableOpacity onPress={() => {
                          const phone = extractPhoneNumber(carWash.contacts);
                          if (phone) {
                            Linking.openURL(`tel:${phone}`);
                          }
                        }}>
                          <Text style={[styles.infoValue, styles.underline]}>{carWash.contacts}</Text>
                        </TouchableOpacity>
                      ) : (
