...
export default function ClothingRepairNewAfonScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/clothing-repair/page/city_page/${encodeURIComponent(t('cities.new_afon'))}/`), { cache: 'no-store' });
