...
export default function HotelsNewAfonScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/hotels/page/city_page/${encodeURIComponent('Новый Афон')}/`, { cache: 'no-store' });
