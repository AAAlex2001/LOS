...
export default function GasStationsNewAfonScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/gas-stations/page/city_page/${encodeURIComponent(t('cities.new_afon'))}/`), { cache: 'no-store' });
