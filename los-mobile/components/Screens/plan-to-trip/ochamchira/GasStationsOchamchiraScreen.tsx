...
export default function GasStationsOchamchiraScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/gas-stations/page/city_page/${encodeURIComponent(t('cities.ochamchira'))}/`), { cache: 'no-store' });
              ...
              <View key={station.id} style={styles.card}>
                {station.image_url && (
                  <View style={styles.imageContainer}>
                    <Image source={{ uri: toImageUrl(station.image_url) }} style={styles.cardImage} contentFit="cover" />
                  </View>
                )}
                
                <View style={styles.infoContainer}>
                  {station.name && (station.name_link ? (
                    <TouchableOpacity onPress={() => openLink(station.name_link)}>
                      <Text style={[styles.stationName, styles.underline]}>{station.name}</Text>
                    </TouchableOpacity>
                  ) : (
...
const styles = StyleSheet.create({
  ...
  stationName: { fontFamily: 'Inter', fontWeight: '800', fontSize: 14, lineHeight: 17, color: '#1129BD' },
