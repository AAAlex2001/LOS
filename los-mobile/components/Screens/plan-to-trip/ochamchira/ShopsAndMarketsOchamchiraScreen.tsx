...
interface ShopsAndMarketsPageData {
  title: string;
  city?: City;
  shops: ShopOrMarket[];
}
...
export default function ShopsAndMarketsOchamchiraScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/shops-and-markets/page/city_page/${encodeURIComponent('Очамчыра')}/`, { cache: 'no-store' });
  ...
  const shops = (pageData?.shops || []).slice().sort((a, b) => a.order - b.order);
              ...
              <View key={shop.id} style={styles.card}>
                {shop.image_url && (
                  <View style={styles.imageContainer}>
                    <Image source={{ uri: toImageUrl(shop.image_url) }} style={styles.cardImage} contentFit="cover" />
                  </View>
                )}
                
                <View style={styles.infoContainer}>
                  {shop.name && <Text style={styles.shopName}>{shop.name}</Text>}
                  ...
                  {shop.contacts && (
                    <View style={styles.infoBlock}>
                      ...
                      {extractPhoneNumber(shop.contacts) ? (
                        <TouchableOpacity onPress={() => {
                          const phone = extractPhoneNumber(shop.contacts);
                          if (phone) {
                            Linking.openURL(`tel:${phone}`);
                          }
                        }}>
                          <Text style={[styles.infoValue, styles.underline]}>{shop.contacts}</Text>
                        </TouchableOpacity>
                      ) : (
                        <Text style={styles.infoValue}>{shop.contacts}</Text>
                      )}
                    </View>
                  )}
                </View>
...
const styles = StyleSheet.create({
  ...
  shopName: { fontFamily: 'Inter', fontWeight: '800', fontSize: 14, lineHeight: 17, color: '#1129BD' },
