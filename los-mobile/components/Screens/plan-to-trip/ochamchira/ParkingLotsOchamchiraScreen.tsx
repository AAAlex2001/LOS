...
export default function ParkingLotsOchamchiraScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/parking-lots/page/city_page/${encodeURIComponent(t('cities.ochamchira'))}/`), { cache: 'no-store' });
              ...
              <View key={parking.id} style={styles.card}>
                {parking.image_url && (
                  <View style={styles.imageContainer}>
                    <Image source={{ uri: toImageUrl(parking.image_url) }} style={styles.cardImage} contentFit="cover" />
                  </View>
                )}
                
                <View style={styles.infoContainer}>
                  {parking.name && <Text style={styles.parkingName}>{parking.name}</Text>}
...
const styles = StyleSheet.create({
  ...
  parkingName: { fontFamily: 'Inter', fontWeight: '800', fontSize: 14, lineHeight: 17, color: '#1129BD' },
