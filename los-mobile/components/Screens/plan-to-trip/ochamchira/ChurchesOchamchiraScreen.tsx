...
export default function ChurchesOchamchiraScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/churches/page/city_page/${encodeURIComponent(t('cities.ochamchira'))}/`), { cache: 'no-store' });
  ...
  const churches = (pageData?.churches || []).slice().sort((a, b) => a.order - b.order);
              ...
              <View key={church.id} style={styles.card}>
                {church.image_url && (
                  <View style={styles.imageContainer}>
                    <Image source={{ uri: toImageUrl(church.image_url) }} style={styles.cardImage} contentFit="cover" />
                  </View>
                )}
                
                <View style={styles.infoContainer}>
                  {church.name && (church.name_link ? (
                    <TouchableOpacity onPress={() => openLink(church.name_link)}>
                      <Text style={[styles.churchName, styles.underline]}>{church.name}</Text>
                    </TouchableOpacity>
                  ) : (
...
const styles = StyleSheet.create({
  ...
  churchName: { fontFamily: 'Inter', fontWeight: '800', fontSize: 14, lineHeight: 17, color: '#1129BD' },
