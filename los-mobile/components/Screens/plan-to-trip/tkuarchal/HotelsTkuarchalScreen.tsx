...
export default function HotelsTkuarchalScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/hotels/page/city_page/${encodeURIComponent('Ткуарчал')}/`, { cache: 'no-store' });
              ...
              <View key={hotel.id} style={styles.card}>
                {hotel.image_url && (
                  <View style={styles.imageContainer}>
                    <Image source={{ uri: toImageUrl(hotel.image_url) }} style={styles.cardImage} contentFit="cover" />
                  </View>
                )}
                
                <View style={styles.infoContainer}>
                  {hotel.name && <Text style={styles.hotelName}>{hotel.name}</Text>}
                  ...
                  {hotel.contacts && (
                    <View style={styles.infoBlock}>
                      ...
                      {extractPhoneNumber(hotel.contacts) ? (
                        <TouchableOpacity onPress={() => {
                          const phone = extractPhoneNumber(hotel.contacts);
                          if (phone) {
                            Linking.openURL(`tel:${phone}`);
                          }
                        }}>
                          <Text style={[styles.infoValue, styles.underline]}>{hotel.contacts}</Text>
                        </TouchableOpacity>
                      ) : (
                        <Text style={styles.infoValue}>{hotel.contacts}</Text>
                      )}
                    </View>
                  )}
...
const styles = StyleSheet.create({
  ...
  hotelName: { fontFamily: 'Inter', fontWeight: '800', fontSize: 14, lineHeight: 17, color: '#1129BD' },
