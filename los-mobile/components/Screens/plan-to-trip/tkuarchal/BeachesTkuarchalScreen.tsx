...
export default function BeachesTkuarchalScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/beaches/page/city_page/${encodeURIComponent('Ткуарчал')}/`, { cache: 'no-store' });
