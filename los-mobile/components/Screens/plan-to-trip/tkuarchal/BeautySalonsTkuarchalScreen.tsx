...
interface BeautySalonsPageData {
  title: string;
  city?: City;
...
export default function BeautySalonsTkuarchalScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/beauty-salons/page/city_page/${encodeURIComponent(t('cities.tkuarchal'))}/`), { cache: 'no-store' });
  ...
  const beautySalons = (pageData?.beauty_salons || []).slice().sort((a, b) => a.order - b.order);
              ...
              <View key={salon.id} style={styles.card}>
                {salon.image_url && (
                  <View style={styles.imageContainer}>
                    <Image source={{ uri: toImageUrl(salon.image_url) }} style={styles.cardImage} contentFit="cover" />
                  </View>
                )}
                
                <View style={styles.infoContainer}>
                  {salon.name && (salon.name_link ? (
                    <TouchableOpacity onPress={() => openLink(salon.name_link)}>
                      <Text style={[styles.salonName, styles.underline]}>{salon.name}</Text>
                    </TouchableOpacity>
                  ) : (
...
const styles = StyleSheet.create({
  ...
  salonName: { fontFamily: 'Inter', fontWeight: '800', fontSize: 14, lineHeight: 17, color: '#1129BD' },
