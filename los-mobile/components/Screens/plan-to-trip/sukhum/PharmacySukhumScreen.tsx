...
export default function PharmacySukhumScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/pharmacy/page/city_page/${encodeURIComponent(t('cities.sukhum'))}/`), { cache: 'no-store' });
      ...
      <View style={styles.container}>
        ...
        <View style={styles.headerBar}>
          ...
          <View style={styles.headerTitleWrap}>
            <Text style={styles.headerTitle}>{pageData?.title ? pageData.title.toUpperCase() : t('categories.pharmacies')}</Text>
          </View>
          <View style={{ width: 36 }} />
        </View>
