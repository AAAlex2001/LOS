...
export default function PharmacySukhumScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/pharmacy/page/city_page/${encodeURIComponent('Сухум')}/`, { cache: 'no-store' });
              ...
              <View key={pharmacy.id} style={styles.card}>
                ...
                {pharmacy.image_url && (
                  <View style={styles.imageContainer}>
                    <Image 
                      source={{ uri: toImageUrl(pharmacy.image_url) }}
                ...
                <View style={styles.infoContainer}>
                  ...
                  {pharmacy.contacts && (
                    <View style={styles.infoBlock}>
                      ...
                      {extractPhoneNumber(pharmacy.contacts) ? (
                        <TouchableOpacity onPress={() => {
                          const phone = extractPhoneNumber(pharmacy.contacts);
                          if (phone) {
                            Linking.openURL(`tel:${phone}`);
                          }
                        }}>
                          <Text style={[styles.infoValue, styles.underline]}>{pharmacy.contacts}</Text>
                        </TouchableOpacity>
                      ) : (
                        <Text style={styles.infoValue}>{pharmacy.contacts}</Text>
                      )}
                    </View>
                  )}
                </View>
