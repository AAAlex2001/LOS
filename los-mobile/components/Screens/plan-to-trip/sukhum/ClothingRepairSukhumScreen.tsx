...
export default function ClothingRepairSukhumScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/clothing-repair/page/city_page/${encodeURIComponent(t('cities.sukhum'))}/`), { cache: 'no-store' });
              ...
              <View key={repair.id} style={styles.card}>
                ...
                {repair.image_url && (
                  <View style={styles.imageContainer}>
                    <Image 
                      source={{ uri: toImageUrl(repair.image_url) }}
