...
export default function BeautySalonsSukhumScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/beauty-salons/page/city_page/${encodeURIComponent(t('cities.sukhum'))}/`), { cache: 'no-store' });
              ...
              <View key={salon.id} style={styles.card}>
                ...
                {salon.image_url && (
                  <View style={styles.imageContainer}>
                    <Image 
                      source={{ uri: toImageUrl(salon.image_url) }}
