...
export default function CulturalAttractionsSukhumScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`${API_BASE}/api/cultural-attractions/page/city_page/${encodeURIComponent('Сухум')}/`, { cache: 'no-store' });
              ...
              <View key={attraction.id} style={styles.card}>
                ...
                {attraction.image_url && (
                  <View style={styles.imageContainer}>
                    <Image 
                      source={{ uri: toImageUrl(attraction.image_url) }}
