...
export default function AdministrativeBuildingsSukhumScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/administrative-buildings/page/city_page/${encodeURIComponent(t('cities.sukhum'))}/`), { cache: 'no-store' });
              ...
              <View key={building.id} style={styles.card}>
                ...
                {building.image_url && (
                  <View style={styles.imageContainer}>
                    <Image 
                      source={{ uri: toImageUrl(building.image_url) }}
