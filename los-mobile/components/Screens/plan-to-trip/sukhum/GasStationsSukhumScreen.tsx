...
export default function GasStationsSukhumScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/gas-stations/page/city_page/${encodeURIComponent(t('cities.sukhum'))}/`), { cache: 'no-store' });
              ...
              <View key={station.id} style={styles.card}>
                ...
                {station.image_url && (
                  <View style={styles.imageContainer}>
                    <Image 
                      source={{ uri: toImageUrl(station.image_url) }}
