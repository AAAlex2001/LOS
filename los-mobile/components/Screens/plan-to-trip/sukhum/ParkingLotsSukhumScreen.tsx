...
export default function ParkingLotsSukhumScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/parking-lots/page/city_page/${encodeURIComponent(t('cities.sukhum'))}/`), { cache: 'no-store' });
              ...
              <View key={parking.id} style={styles.card}>
                ...
                {parking.image_url && (
                  <View style={styles.imageContainer}>
                    <Image 
                      source={{ uri: toImageUrl(parking.image_url) }}
