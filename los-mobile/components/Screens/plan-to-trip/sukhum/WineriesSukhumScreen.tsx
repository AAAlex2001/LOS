...
export default function WineriesSukhumScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/wineries/page/city_page/${encodeURIComponent(t('cities.sukhum'))}/`), { cache: 'no-store' });
      ...
      <View style={styles.container}>
        ...
        <View style={styles.headerBar}>
          ...
          <View style={styles.headerTitleWrap}>
            <Text style={styles.headerTitle}>{pageData?.title ? pageData.title.toUpperCase() : t('categories.wineries')}</Text>
          </View>
          <View style={{ width: 36 }} />
        </View>
              ...
              <View key={winery.id} style={styles.card}>
                ...
                {winery.image_url && (
                  <View style={styles.imageContainer}>
                    <Image 
                      source={{ uri: toImageUrl(winery.image_url) }}
                  ...
                  {winery.contacts && (
                    <View style={styles.infoBlock}>
                      ...
                      <Text style={styles.infoValue}>
                        {parseContactString(winery.contacts).map((segment, index) => {
