...
export default function ParkingLotsGalScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/parking-lots/page/city_page/${encodeURIComponent(t('cities.gal'))}/`), { cache: 'no-store' });
