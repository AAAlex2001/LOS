...
export default function WineriesGalScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/wineries/page/city_page/${encodeURIComponent(t('cities.gal'))}/`), { cache: 'no-store' });
