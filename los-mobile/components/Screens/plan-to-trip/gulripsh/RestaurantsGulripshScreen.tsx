...
export default function RestaurantsGulripshScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(addLangParam(`${API_BASE}/api/restaurants/page/city_page/${encodeURIComponent(t('cities.gulripsh'))}/`), { cache: 'no-store' });
