...
class RestaurantInline(TranslationTabularInline):
    model = Restaurant
    extra = 1
    fields = ("city", "name", "name_link", "website", "address", "address_link", "phone", "working_hours", "image", "order", "preview")
    ...
    def get_parent_object(self, request):
        ...
        if not object_id:
            return None
        return RestaurantsPage.objects.filter(pk=object_id).first()

    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "city":
            page = self.get_parent_object(request)
            if page is not None:
                kwargs["queryset"] = RestaurantCity.objects.filter(page=page)
        return super().formfield_for_foreignkey(db_field, request, **kwargs)


class RestaurantCityInline(TranslationTabularInline):
...
class RestaurantsPageAdmin(TranslationAdmin):
    list_display = ("id", "updated_at")
    inlines = [RestaurantCityInline, RestaurantInline]
    readonly_fields = ("seo_preview",)

    def has_add_permission(self, request):
        if RestaurantsPage.objects.exists():
            return False
...
## ВАЖНО: Не регистрируем модели `RestaurantCity` и `Restaurant` отдельно,
