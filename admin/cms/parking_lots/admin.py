...
class ParkingLotInline(TranslationTabularInline):
    model = ParkingLot
    extra = 1
    ...
    def get_parent_object(self, request):
        ...
        if not object_id:
            return None
        return ParkingLotsPage.objects.filter(pk=object_id).first()

    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "city":
            page = self.get_parent_object(request)
            if page is not None:
                kwargs["queryset"] = ParkingLotCity.objects.filter(page=page)
        return super().formfield_for_foreignkey(db_field, request, **kwargs)


class ParkingLotCityInline(TranslationTabularInline):
...
class ParkingLotsPageAdmin(TranslationAdmin):
    list_display = ("id", "updated_at")
    inlines = [ParkingLotCityInline, ParkingLotInline]
    readonly_fields = ("seo_preview",)

    def has_add_permission(self, request):
        if ParkingLotsPage.objects.exists():
            return False
...
## ВАЖНО: Не регистрируем модели `ParkingLotCity` и `ParkingLot` отдельно,
