...
class AdministrativeBuildingInline(TranslationTabularInline):
    ...
    def get_parent_object(self, request):
        # Получаем текущую страницу из URL админки: /admin/cms/administrativebuildingspage/<object_id>/change/
        try:
        ...
        if not object_id:
            return None
        return AdministrativeBuildingsPage.objects.filter(pk=object_id).first()

    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        # В поле "city" показываем только города текущей страницы
        if db_field.name == "city":
            page = self.get_parent_object(request)
            if page is not None:
                kwargs["queryset"] = AdministrativeBuildingCity.objects.filter(page=page)
...
## ВАЖНО: Не регистрируем модели `AdministrativeBuildingCity` и `AdministrativeBuilding` отдельно,
